...
for j in ['lr','rf','svm']:
    for name in ['rew','fairway','fairsmote','maat']:
        for dataset in ['Adult-Sex','Adult-Race','Compas-Sex','Compas-Race','German-Sex','Bank-Age','Mep-Race']:
            (dataset_pre,dataset_aft) = dataset.lower().split('-')
            if dataset == 'Mep-Race':
                dataset_pre = 'mep'
            ...
            fin.close()
    for name in ['m-rew','m-fairway','m-fairsmote']:
        for dataset in ['Adult-Sex','Adult-Race','Compas-Sex','Compas-Race','German-Sex','Bank-Age','Mep-Race']:
            (dataset_pre,dataset_aft) = dataset.lower().split('-')
            if dataset == 'Mep-Race':
                dataset_pre = 'mep'
                dataset_aft = 'RACE'
            fin = open('../RQ3_results/'+name+'_'+j+'_'+dataset_pre+'_'+dataset_aft+'.txt','r')
            count = 0
            for line in fin:
                count=count+1
                if count in data_key_value_used:
    ...
    for fairmetric in ['SPD','AOD','EOD']:
        ...
        for permetric in ['Acc','Mac-P','Mac-R','Mac-F1','MCC']:
            ...
            for algo in ['rf','lr','svm']:
                ...
                for name in ['rew','fairway','fairsmote','m-rew','m-fairway','m-fairsmote','maat']:
